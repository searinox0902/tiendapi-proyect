...
>
> **Costo de adquisición (`provider_price`, D-47) — lado opuesto, no confundir con `base_price`.** `base_price`/`sale_price` son lo que el negocio **cobra** (lado venta); `provider_price` es lo que el negocio **paga** al `provider_id` principal para conseguirlo (lado compra) — dos números independientes que nunca se derivan uno del otro. A diferencia de `sale_price`, **no es una fórmula**: es un valor de entrada editable directamente, que sirve como costo **sugerido/por defecto** al crear nuevos Ítems de esta Referencia. El costo real de cada unidad física vive en `Item.provider_price` (1.3), porque puede variar por distribuidor puntual. Mecanismo de captura (manual vs. XML del proveedor) sin decidir — ver A-25 en [07](07-decisiones-y-puntos-abiertos.md).

> **`sku` — clave natural única por tenant** (índice `ix_product_references_tenant_sku`, migración `0018`, D-90). El SKU es la identidad de negocio de la Referencia y la clave por la que se serializa al importar/exportar (criterio de D-42: por nombre natural, no por UUID).
>
> **D-90 revocó la unicidad *blanda* que D-73 había fijado a propósito.** El argumento original —que un lote de mil productos no se caiga por un duplicado— seguía siendo válido, pero la tolerancia no era inocua: con dos Referencias del mismo SKU, `GET /references/lookup` (el endpoint que la caja usa para autocompletar por código) devolvía **HTTP 500**. Hoy el candado está en la base y el lote sigue sin caerse, porque un código repetido tiene salida propia: se salta o se convierte en **variante**.
>
> **Variantes — convención en el propio código, sin columna ni FK (D-90).** Cuando un mismo código de proveedor cubre piezas realmente distintas, la variante es una Referencia **completa** con SKU propio (`41080-0578-11H#2`); lo único que comparte con sus hermanas es la **base** del código, y de ahí sale la presentación agrupada. El `#` es infalsificable: `normalize_sku` lo convierte en `-` salvo que venga como `#<número>` al final, así que solo el sistema lo produce.
>
> **SKU generado (`GEN-`, D-86) y normalizado (D-85).** Una fila sin código recibe `GEN-` + 8 hex determinista sobre título+marca, para que reimportar el mismo archivo no duplique; los separadores (espacios, `_`, `*`, `,`, `.`, `#`) se unifican a `-` al **escribir**, nunca al leer.
...
### 1.3 Ítem (`Item`)
**Una unidad física individual** de una Referencia — no un lote ni una cantidad. Cada `Item` es una fila propia, con su propio `id` (identidad única), su propia sucursal/bodega (FK a `Location`, ver 1.8), su propio precio de venta, su propio proveedor de origen y su propio estado (`available | sold | reserved | written_off`). Una Referencia puede tener **N Ítems** — ej. "Aceite Motul" con 24 unidades en stock son **24 filas `Item`**, cada una identificable y editable por separado (D-41).

> **Precio de venta de la unidad (`current_price`, D-45):** es el **precio de venta con IVA de ESTA unidad**. Se **inicializa** como espejo de `Reference.sale_price` al crear el Ítem, y es el **único lugar donde vive el ajuste humano** (descuento puntual a una unidad, D-41): es **mutable mientras `status = 'available'`** y se **congela al pasar a `sold`**. Si nadie lo toca, al vender coincide con el precio de catálogo; si se editó (ej. "te lo dejo en $38.000"), diverge — y esa divergencia es justo lo que permite reportar descuentos otorgados sin reconstruir nada. El desglose base/IVA de esa venta **no** se guarda aquí, se deriva a la inversa en la línea de factura (1.6).
>
> **Costo real de la unidad (`provider_price`, D-47).** Se **inicializa** como espejo de `Reference.provider_price` (el costo sugerido del proveedor principal), pero es **editable independientemente por unidad** — la razón de fondo: la misma Referencia/SKU puede tener Ítems que llegaron de **distribuidores distintos** a costos distintos (ya soportado por `Item.provider_id`, que puede diferir del proveedor principal de la Referencia). Es lo que habilita el cálculo real de utilidad: `utilidad = (BillItem.unit_price − Item.provider_price) × quantity`. No tiene relación con `current_price`/IVA — es un dato de costo puro, nunca se le aplica IVA.
...
> **Carga por archivo — una fila del archivo son N unidades (D-92, resuelve A-30).** El importador de Productos (`POST /items/import`) traduce la columna `existencias` a **N filas de `Item`**, coherente con D-41: el sistema cuenta filas (`func.count(Item.id)`), nunca suma `Item.quantity` — que queda como columna muerta. La frontera con el catálogo es la que fija D-92: con el mismo SKU, **proveedor y ubicación distintos son unidades** (son ejes del Ítem, no de la Referencia), mientras que un **nombre distinto es otra pieza** y se crea como variante `#N` (D-90). El archivo **nunca edita una Referencia existente**: marca, categoría y precios que difieran se reportan en la previsualización y se ignoran. Las unidades nacen **sin overrides**, o sea siguiendo el precio vigente del catálogo vía `COALESCE`. `items.import_batch_id` (migración `0019`) etiqueta el lote para poder deshacerlo entero.
...
> **Estado fiscal (`fiscal_status`, D-48, ampliado por D-59) — columna creada y ya consumida por la UI.** Columna nullable (`contingencia | pendiente | autorizada | rechazada`) que anticipa el flujo de facturación electrónica DIAN (D-38): una factura emitida offline entra en `contingencia`/`pendiente` y pasa a `autorizada` al recibir el CUFE del `FiscalRecord` remoto. **Sin integración DIAN (diferida, D-09) el campo queda nulo** en operación real. Implementada en la migración `0009_bill_fiscal_status` con `CHECK` en base (mismo criterio que `ck_locations_type`, D-25). **D-59 revisa el encuadre original de D-48:** el desglose declaradas / sin declarar sí aparece en la fila de métricas de Facturación (card "N.º de facturas"), además del filtro por estado; el banner de alerta de cumplimiento sigue pendiente para cuando la integración esté viva. `NULL` se cuenta aparte de "sin declarar" a propósito — no es incumplimiento, es que el trámite no existe todavía.

> **Anulación (`voided_at` / `void_reason`, D-60) — migración `0010_bill_void`.** Anular es **marcar, no borrar**: la factura anulada conserva su consecutivo y sus líneas, porque un documento numerado que desaparece deja un hueco en la serie que después nadie puede explicar (y `_next_bill_number` se calcula del máximo existente, así que borrarlo tampoco liberaría el número). La anulación tiene dos efectos, ambos en la misma transacción del servidor: (1) la factura **deja de sumar** a total facturado, IVA recaudado y utilidad (D-49) — el resumen la sigue contando como documento emitido en `total_bills` y la reporta aparte en `voided_bills`; (2) **todas** las unidades de la factura **vuelven al inventario** como `available`, sin mirar en qué estado están hoy (D-65). El motivo es obligatorio en el endpoint: un mes después, "anulada" a secas no distingue una devolución de un error de digitación. Una factura **`autorizada` por la DIAN no se anula** — se corrige con nota crédito; el endpoint la rechaza con 409.
...
**Venta libre (D-42) — ⚠️ especificada pero NO implementada (A-27).** *Diseño:* `item_id` es **opcional**; si la línea es de algo no catalogado (ítem a peso/volumen, o cualquier cosa sin Referencia previa), `item_id` queda nulo y se llena `description` (texto libre) en su lugar — siempre uno de los dos, nunca ambos vacíos ni ambos llenos. La factura sigue calculando subtotal/IVA/total igual; esa venta simplemente **no descuenta ningún inventario**.

> **Estado real del código:** `item_id` es `nullable=False`, **no existe la columna `description`** en `bill_items` y no hay `CHECK`. Hoy **no se puede facturar nada que no esté catalogado** — justo el caso que D-42 argumentaba. Detectado al diseñar D-73 (la ausencia de venta libre es lo que hace que *toda* factura bloquee el reemplazo de catálogo). Falta decidir si se construye o se retira: **A-27**.

### 1.7 Categoría (`Category`) — nueva entidad
Agrupa Referencias por tipo de producto (ej. "Lubricantes", "Ferretería"). Antes se referenciaba como `category_id` en `Reference` sin tabla propia; ahora es una entidad formal con `id`, `name`, `description` e **`icon`** (D-72: nombre de ícono lucide, ej. `"wrench"` — string libre, **sin** `enum`/`CHECK` en el backend a propósito, porque el set de íconos válidos vive y crece en el frontend, `frontend/src/lib/categoryIcons.ts`; un nombre no reconocido cae a un ícono por defecto en vez de romper la UI).
...
### 1.9 Marca (`Brand`) — nueva entidad, desacoplada de `Reference.brand`

Catálogo propio de marcas (NGK, Brembo, Bosch), con `id`, `name` y `description` — vive en el Directorio (D-71), junto a Proveedor/Ubicación/Categoría/Cliente. **Deliberadamente no es una FK de `Reference`:** `Reference.brand` (§1.2) sigue siendo texto libre, sin vínculo estructural con esta tabla — D-71 explica el porqué de mantenerlas desacopladas por ahora. `Brand` se alimentó por *backfill* de los valores distintos que ya existían en `Reference.brand` al migrar (copia inicial, no sincronización continua).

### 1.10 Columnas transversales de integridad y sincronización
...
Table Reference {
  ...
  provider_price decimal(12,2)            // Costo sugerido/por defecto (lado COMPRA, no confundir con base_price/sale_price que son lado venta). Entrada editable directa, NO derivada. Default al crear nuevos Ítems (D-47)
  created_at timestamp                      // Fecha de creación
  updated_at timestamp
  version int
  synced_at timestamp
...
Table Item {
  ...
  status varchar                            // 'available' | 'sold' | 'reserved' | 'written_off' (D-41)
  current_price decimal(12,2)               // Precio de venta CON IVA de ESTA unidad. Inicializado = Reference.sale_price; mutable mientras status='available', congelado al vender (D-45)
  provider_price decimal(12,2)            // Costo real pagado por ESTA unidad (lado COMPRA). Inicializado = Reference.provider_price, editable independiente por unidad — permite distribuidores distintos al mismo SKU (D-47)
  created_at timestamp                      // Fecha de creación
  updated_at timestamp
  version int
  synced_at timestamp
...
Table Bill {
  ...
  voided_at timestamp [null]                // ANULACIÓN (D-60): nulo = vigente. Anular es marcar, no borrar — el consecutivo se conserva
  void_reason varchar [null]                // Motivo de la anulación (D-60). Obligatorio en el endpoint, nullable en base por las filas previas a la migración 0010
...
- **Imágenes de producto (D-40, esquema reservado, aún no implementado):** cuando se añada (fast-follow tras F2), `Reference` llevará `image_source` (`'local' | 'url' | 'cloud'`) + `image_ref`. Para `local`, `image_ref` es el **SKU de la Referencia** (D-58 supera la naming original por hash SHA-256): el archivo vive en `appDataDir/images/<SKU>.<ext>`, nombrado a mano si el usuario lo desea (manipulable/trazable sin pasar por la UI). Para `url` es la URL externa; para `cloud`, la referencia remota. **Nunca** el binario como BLOB en la base cifrada. No se agrega la columna todavía (YAGNI); solo queda reservado el diseño para que la migración futura sea limpia.
- **Estado fiscal DIAN (`Bill.fiscal_status`, D-48 / D-59):** **implementada** en la migración `0009_bill_fiscal_status` (nullable + `CHECK` en base). Anticipa el flujo de autorización fiscal (contingencia offline → autorizada con CUFE) sin migrar `Bill` (tabla con `hmac`/`prev_hash`) más adelante. Sin la integración DIAN (diferida, D-09/D-38) queda nula en operación real. **D-59** amplía el encuadre de D-48: el desglose declaradas / sin declarar se muestra en la card "N.º de facturas" de la pantalla Facturación y hay filtro por estado; el **banner** de alerta de cumplimiento sigue pendiente de la integración. Ver 1.5.
- **Anulación de facturas (`Bill.voided_at` / `void_reason`, D-60):** **implementada** en la migración `0010_bill_void` (dos columnas nullable + índice sobre `voided_at`, que el resumen de Facturación consulta en cada carga). Anular es marcar, no borrar; la anulación devuelve las unidades al inventario y excluye la factura de todas las cifras de dinero, en una sola transacción. Ver 1.5.
