...
import type { IItemStockUnit, IItemUpdatePayload } from "@/api/items/items.types"
 ...
 */
function uniformValue(ids: string[], stockUnits: IItemStockUnit[], getValue: (e: IItemStockUnit) => string | null) {
  const selected = stockUnits.filter(stockUnit => ids.includes(stockUnit.item_id))
  if (selected.length === 0) return NONE
  const first = getValue(selected[0])
  const allShareValue = selected.every(stockUnit => getValue(stockUnit) === first)
 ...
 */
function uniformDecimalValue(ids: string[], stockUnits: IItemStockUnit[], getValue: (e: IItemStockUnit) => string | null): string {
  const selected = stockUnits.filter(stockUnit => ids.includes(stockUnit.item_id))
  if (selected.length === 0) return ""
  const first = getValue(selected[0])
  if (first === null) return ""
  const allShareValue = selected.every((stockUnit) => {
    const value = getValue(stockUnit)
...
watch(open, (isOpen) => {
  ...
  providerValue.value = uniformValue(props.itemIds, props.stockUnits, stockUnit => stockUnit.provider_id)
  locationValue.value = uniformValue(props.itemIds, props.stockUnits, stockUnit => stockUnit.location_id)
  providerInitial.value = providerValue.value
  locationInitial.value = locationValue.value

  basePriceInput.value = uniformDecimalValue(props.itemIds, props.stockUnits, stockUnit => stockUnit.base_price)
  ivaInput.value = uniformDecimalValue(props.itemIds, props.stockUnits, stockUnit => stockUnit.iva_percentage)
  providerPriceInput.value = uniformDecimalValue(props.itemIds, props.stockUnits, stockUnit => stockUnit.provider_price)
  ...
  <Dialog v-model:open="open">
    <DialogContent class="sm:max-w-2xl">
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        -->
        <div class="grid gap-2">
          <Label>Precio proveedor (sin IVA)</Label>
          <div class="relative">
